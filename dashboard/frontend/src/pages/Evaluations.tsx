  ...
  return <div className="page-stack">
    <SectionHeading title="Evaluations" description="Compare defense effectiveness, task utility, baseline behavior and ablations."
      action={<DownloadButton label="Export comparison" onClick={() => downloadCSV('sentinel-evaluations.csv',
        ['Experiment', 'Kind', 'Scenarios', 'Legitimate task rate', 'Attack success rate', 'Critical violation rate', 'False block rate', 'Unnecessary escalation rate', 'Mean latency (ms)'],
        filtered.map((item) => [item.name, item.kind, item.totalScenarios, item.legitimateTaskRate, item.attackSuccessRate, item.criticalViolationRate, item.falseBlockRate, item.unnecessaryEscalationRate, item.averageLatencyMs]))} />} />
    ...
    <Panel title="Experiment comparison" action={<div className="segmented-control" role="group" aria-label="Experiment type filter">
      ...
      {filtered.length === 0 ? <EmptyState title="No experiments" description="No experiments match the selected filter." /> : <div className="table-scroll"><table className="data-table evaluation-table">
        <thead><tr><th>Experiment</th><th>Kind</th><th>Scenarios</th><th>Task utility</th><th>Attack success</th><th>Critical violations</th><th>False blocks</th><th>Escalation</th><th>Avg. latency</th></tr></thead>
        <tbody>{filtered.map((item) => <tr key={item.id} className={item.kind === 'full' ? 'evaluation-row--primary' : ''}>
          ...
          <td>{asPercent(item.criticalViolationRate)}</td><td>{asPercent(item.falseBlockRate)}</td><td>{asPercent(item.unnecessaryEscalationRate)}</td><td>{item.averageLatencyMs === null ? '—' : `${item.averageLatencyMs} ms`}</td>
        </tr>)}</tbody>
      </table></div>}
    </Panel>
