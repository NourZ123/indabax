...
export function PoliciesPage({ policies, runs }: { policies: PolicyDocument[]; runs: Run[] }) {
  ...
  return <div className="page-stack">
    ...
    <div className="policy-summary-grid">
      <Panel title="Active policy"><div className="policy-headline"><div className="policy-icon"><ShieldCheck size={23} /></div><div><h3>{policy.name}</h3><p>{policy.description}</p></div></div><div className="policy-meta-grid"><Field label="Policy ID" value={<code>{policy.id}</code>} /><Field label="Version" value={policy.version} /><Field label="Tools listed" value={policy.tools.length} /><Field label="Rules listed" value={policy.rules.length} /></div></Panel>
      <Panel title="Enforcement model"><div className="enforcement-summary"><div><StatusDot status="online" label="Read-only inspection" /><p>Each candidate action is checked against the active scenario's exposed policy context. These illustrative documents are not a substitute for a run's exact policy.</p></div><div className="enforcement-divider" /><div><strong>Safety controls</strong><span>Tool permission · Confirmation · Data flow · Prerequisites · Forbidden effects</span></div></div></Panel>
    </div>
    ...
    <div className="policy-rules-grid">
      ...
      <Panel title="Rule inspector">{rule ? <div className="rule-inspector"><span className="eyebrow">{rule.id} · {rule.kind.replaceAll('_', ' ')}</span><h3>{rule.name}</h3><div className="rule-severity"><span className={`severity-marker severity-marker--${rule.severity}`} />{rule.severity} severity</div>
        ...
        <div className="inspect-section"><h4>Related decision signals</h4><p className="help-copy">Matched by reason-code category; without explicit rule IDs in a trace, these are not verified rule-specific enforcement events.</p>
          {related.length === 0 ? <p className="muted">No related signals appear in the available demo runs.</p> : <div className="related-events">{related.slice(0, 4).map(({ run, event }) => <Link to={`/runs/${run.id}`} key={event.id}><span><strong>{run.id}</strong> · {event.tool}</span><StatusDot status={event.decision} /></Link>)}</div>}
